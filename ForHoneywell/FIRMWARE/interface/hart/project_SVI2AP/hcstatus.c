...
u8 hart_StatusPlugin(u8 rsp2)
{
    ...
    // according to my understanding of Spec, this bit should be set regardless of hart revision
    // but to be backward compatibility, only set when HART6, by EJ
    if (HART_REV_6 == tmpptr->hart_version)
    {
        if (hart_GetLoopCurModeByDevMode() == (u8)LoopModeDisabled)
        {
            rsp2 |= HART_LOOPCURRENT_FIXED;
        }
    }
#endif
