...
// SP SHALL BE IN THE -50.0 to 160.0 range
#define FSTAT_VALUE_MIN                  (-50.0f)
#define FSTAT_VALUE_MAX                  (160.0f)
...
*/
static bool_t   ff_fstate_FStateRangeCheck(u8 MN_CONSTRANGE(0,NELEM(FState_Ranges)-1) index, float_t ToVerify)
{
    bool_t  result;
...
/*
static float_t  TruncateFloatValue(float_t TimeValue)
{
    s32     TempValue;
...
static float_t  ff_fstate_CheckAndCorrect(u8 index, float_t ToVerify)
{
    float_t result;
...
fferr_t     ff_fstate_VerifyWriteConf(const T_FBIF_BLOCK_INSTANCE *p_block_instance, const T_FBIF_WRITE_DATA *p_write)
{
    ...
    switch (p_write->subindex)
    {
        case 0 : // everything)
            ...
            p_fVal  = (float_t *)(void *)(p_write->source + MN_OFFSETOF(_XD_FSTATE, component_2) - 2u);
...
*/
        ...
        case 3 : // Val
            p_fVal   = (float_t *)(void *)p_write->source;
            ...
            switch (BlockID)
            {
                ...
                case ID_DOFB_2 :
                    {   // Legit FB
                        ...
                        p_PTB->xd_fstate.component_2  = ff_fstate_CheckAndCorrect(FSTATE_VALUE_INDEX, (float_t)p_DOFB->fstate_val_d);
