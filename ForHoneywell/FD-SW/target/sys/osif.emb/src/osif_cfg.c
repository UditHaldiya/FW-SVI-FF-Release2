...
STATIC NO_INIT uint8_t las_stack[LAS_STACK_SIZE];
...
STATIC NO_INIT uint8_t fdc_stack[FDC_STACK_SIZE];
...
STATIC NO_INIT uint8_t sbscr_stack[SBSCR_STACK_SIZE];
...
STATIC NO_INIT uint8_t mvc_stack[MVC_STACK_SIZE];
#endif /* MVC_OBJECTS */
...
STATIC NO_INIT uint8_t bgd50_stack[BGD50_STACK_SIZE];
...
#define las_mb NULL
        #else
static OS_MAILBOX las_mb[LAS_MSGNUM+MB_MORE];
        #endif
    #endif /* LINK_MASTER */
    #if FDC_MSGNUM == 0
#define fdc_mb NULL
    #else
static OS_MAILBOX fdc_mb[FDC_MSGNUM+MB_MORE];
    #endif
    #if SBSCR_MSGNUM == 0
#define sbscr_mb NULL
    #else
static OS_MAILBOX sbscr_mb[SBSCR_MSGNUM+MB_MORE];
    #endif
    #ifdef MVC_OBJECTS
        #if MVC_MSGNUM == 0
#define mvc_mb NULL
        #else
static OS_MAILBOX mvc_mb[MVC_MSGNUM+MB_MORE];
        #endif
    #endif /* MVC_OBJECTS */
    #if BGD50_MSGNUM == 0
#define bgd50_mb NULL
    #else
static OS_MAILBOX bgd50_mb[BGD50_MSGNUM+MB_MORE];
    #endif
#endif /* Protocol type */
    #if BGD_MSGNUM == 0
#define bgd_mb NULL
    #else
static OS_MAILBOX bgd_mb[BGD_MSGNUM+MB_MORE];
    #endif
    #if EEP_MSGNUM == 0
#define eep_mb NULL
    #else
static OS_MAILBOX eep_mb[EEP_MSGNUM+MB_MORE];
    #endif
    #if FB_MSGNUM == 0
#define fb_mb NULL
    #else
static OS_MAILBOX fb_mb[FB_MSGNUM+MB_MORE];
    #endif
    #if defined (HART_MASTER)
        #if HM_MSGNUM == 0
#define hm_mb NULL
