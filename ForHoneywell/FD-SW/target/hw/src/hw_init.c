...
FUNCTION_DECLARATIONS
  ...
  extern void hw_cfg_GPIO();
  extern void hw_cfg_TIMx();
  extern void hw_cfg_EXTI(FunctionalState);
  extern void hw_cfg_NVIC();
  extern void hw_cfg_FSMC();
...
FUNCTION VOID Reset_Watchdog (VOID);
...
__interwork int __low_level_init(void)
{
    ...
    */
    ...
    for (i=0; i<100; i++);                /* Reason __UNKNOWN__!!             */
    Reset_Watchdog();                   /* Reset external Watchdog */
#endif /* FC_SPC42 */
...
void Reset_CPU(void)
{
    ...
    Reset_Watchdog();  //same as APP's wdog_WatchdogTrigger();

    //Replace with HardReset_CPU when all traps infrastructure is in place
    mn_assert((void *)__get_LR(), -TRAPID_SWI); //steal TRAPID_SWI to indicate a reset
}
...
#if USE_WD_INSTRUM != 0
typedef struct WDInstrum_t
{
    u32 first_tickle;
    u32 curr_tickle;
    u32 max_tickle;
} WDInstrum_t;
WDInstrum_t WDInstrum;

/** \brief Calculates the first and the running max between tickles
As written, useless until RTOS system timer is running
*/
static void wdinstrum(void)
{
    MN_ENTER_CRITICAL();
        u32 tm = OS_GetTime32();
        if(WDInstrum.curr_tickle == 0)
        {
            WDInstrum.first_tickle = tm;
        }
        else
        {
            u32 diff = tm - WDInstrum.curr_tickle;
            if(diff > WDInstrum.max_tickle)
            {
                WDInstrum.max_tickle = diff;
            }
        }
        WDInstrum.curr_tickle = tm;
    MN_EXIT_CRITICAL();
}
#endif //USE_WD_INSTRUM
...
Reset/Clear the external Watchdog

MAXIM Datasheet for MAX6301ESA Low-Power uP Supervisory Circuits with Adjustable Reset/Watchdog

 "WDI: Watchdog Input. A rising or falling transition must occur on this input within the selected watchdog
...
FUNCTION_BODY
    ...
    if ((GPIOA->ODR & ((USIGN16)0x0002)) != 0u)
