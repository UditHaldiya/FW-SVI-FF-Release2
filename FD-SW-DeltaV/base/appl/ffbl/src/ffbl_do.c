...
FUNCTION_BODY
  ...
  {
      //before switching from OOS check if no conflict with the other DO channel
      mn_ErrorBits_Set(SP_SOURCE_ERRORBIT, FALSE); //clear the error bit

      if ( mn_ConflictOtherDOChannels(p_block_desc) )
      {
          p_dofb->mode_blk.actual = MODE_OS;
          mn_ErrorBits_Set(SP_SOURCE_ERRORBIT, TRUE); //set error bit if needed
          return (p_dofb->blk_data.next_fb_to_execute);
      }
...
FUNCTION_BODY
  ...
  CONST_ASSERT(SP_SOURCE_ERRORBIT < MN_ERR_CONDITION_MAX);

  bool_t err =  mn_ConflictOtherDOChannels(p_block_desc);
  MN_ENTER_CRITICAL();
      if(err)
      {
          block_err_desc |= (u32)(1u << SP_SOURCE_ERRORBIT); //set error bit if needed
      }
      else
      {
          block_err_desc &= (u32)~(1u << SP_SOURCE_ERRORBIT); //clear error bit if needed
      }
...
static bool_t mn_ConflictOtherDOChannels(const T_FBIF_BLOCK_DESCR *p_block_desc)
{
    ...
    //Hack
    u32 block_err_desc = (1u << SP_SOURCE_ERRORBIT);
    MN_ENTER_CRITICAL();
        if(check)
        {
#if 0
            pOther_block->block_err_desc_1 |= block_err_desc;
#endif
