...
*/
static void bufswap(volatile u16 *pold, u16 *pnew)
{
    if(pnew != NULL)
    {
        u16 newv = *pnew;
...
*/
void buffer_SelectRange(u8_least bufnum, u16 *startpos, u16 *endpos)
{
    if(bufnum < NUM_DIAG_BUFFERS)
    {
        MN_ENTER_CRITICAL();
                          ...
                          u16 InitialPruneScale)
{
    ...
    if(
       ...
       || (fill_func == NULL)
       || (((u16_least)max_points + (u16_least)init_points) >= DIAGNOSTIC_BUFFER_SIZE)
