...
//------------------- watchdog part ----------------------------
typedef u16 wdbitmap_t;
#define tasks_always_monitored ((1U << BGD_TASK_ID) | (1U << EEP_TASK_ID))
#define tasks_never_monitored (1U << APPL_TASK_ID)
typedef struct wdogmon_t
{
    wdbitmap_t tasks_requested;
    wdbitmap_t tasks_checkedin;
    u16 CheckWord;
} wdogmon_t;
static const wdogmon_t wdogmon_init =
{
    .tasks_requested = 0U,
    .tasks_checkedin = 0U,
    .CheckWord = CRC_SEED,
};
static wdogmon_t wdogmon =
{
    .tasks_requested = 0U,
    .tasks_checkedin = 0U,
    .CheckWord = CRC_SEED,
...
//AK: Instrumentation of watchdog tickle
#if USE_WDT_INSTRUM != 0
typedef struct WDTInstrum_t
{
    u32 first_tickle;
    u32 curr_tickle;
    u32 max_tickle;
...
WDTInstrum_t WDTInstrum;

static void wdtinstrum(u8 taskid)
{
    u32 tm = OS_GetTime32();
    if(WDTInstrum.curr_tickle == 0)
    {
        WDTInstrum.first_tickle = tm;
        WDTInstrum.first_task = taskid;
    }
    else
    {
        u32 diff = tm - WDTInstrum.curr_tickle;
        if(diff > WDTInstrum.max_tickle)
        {
            WDTInstrum.max_tickle = diff;
            WDTInstrum.last_task = taskid;
        }
    }
    WDTInstrum.curr_tickle = tm;
}
#else
#define wdtinstrum(taskid) /*empty*/
#endif //USE_WDT_INSTRUM

void osif_TickleWatchdog(u8 taskid)
{
    MN_ENTER_CRITICAL();
#if 0
        //For now, tickle the watchdog if eep task checked in
        if(taskid == EEP_TASK_ID)
        {
            Reset_Watchdog();
        }
#endif
        wdbitmap_t tasks_checkedin = wdogmon.tasks_checkedin | (1U << taskid);
        wdbitmap_t tasks_expected = (wdogmon.tasks_requested | tasks_always_monitored) & ~tasks_never_monitored;
        if(tasks_expected == (tasks_expected & tasks_checkedin))
        {
            //All expected tasks checked in
            wdtinstrum(taskid);
            Reset_Watchdog();
            wdogmon = wdogmon_init;
        }
        else
        {
            //Just check the task in
            storeMemberInt(&wdogmon, tasks_checkedin, tasks_checkedin);
        }
    MN_EXIT_CRITICAL();
}


static void osif_NotifyWatchdog(u8 taskid)
{
    MN_ENTER_CRITICAL();
        wdbitmap_t tasks_requested = wdogmon.tasks_requested | (1U << taskid);
        storeMemberInt(&wdogmon, tasks_requested, tasks_requested);
    MN_EXIT_CRITICAL();
}

/** \brief sets the watchdog trap record with the last WD refresh flags
*/
void process_TrapWatchdog(void)
{
    mn_assert(HARDWARE(u8 *, (wdogmon.tasks_requested <<16)|wdogmon.tasks_checkedin), -TRAPID_WATCHDOG);
}
...
static void osif_TaskRunner(void *tptr)
{
    ...
    osif_NotifyWatchdog(pTask->task_id);
