...
*/
/**
    \file mn_travel_accumulation.c
    \brief "travel_accumulation parameters of transduser block" handler
...
*/
/* $History: mn_travel_accumulation.c $
 *
 * *****************  Version 1  *****************
...
#define MAX_DEADBAND                    (10.0f)
#define MIN_DEADBAND                    (0.05f)

/* \brief synchronize the value of deadband in TRAVEL_ACCUMULATION_A and TRAVEL_ACCUMULATION_B
    param in:
    ...
    return:
        FF error
*/
fferr_t SyncTravelDeadBand(T_FBIF_BLOCK_INSTANCE* p_block_instance, const T_FBIF_WRITE_DATA* p_write)
{
    fferr_t fferr = E_OK;
    ...
    if(3 == p_write->subindex || 0 == p_write->subindex)
    {
        ...
        if(deadband > MAX_DEADBAND || deadband < MIN_DEADBAND)
        {
        ...
        else
        {
            ...
            if(REL_IDX_PTB_TRAVEL_ACCUMULATION_A_ALERT == p_write->rel_idx)
            {
