...
FUNCTION_BODY
  ...
  if (p_write->phase == PRE_WRITE_CHECK)
  {
    ...
    {
      /* The 'restart' parameter should be written -------------------------- */
      if (*p_write->source == RS_RESTART_WITH_DEFAULTS)
      {
      ...
      else if (*p_write->source == RS_RESTART_PROCESSOR)
      {
      ...
      else if (*p_write->source == RS_RESTART_FACTORY_DEFAULTS)
      {
  ...
  else /* p_write->phase == POST_WRITE_ACTION */
  {
    if (p_write->rel_idx == REL_IDX_RESB_RESTART)
    {
      if (*p_write->source == RS_RESTART_FACTORY_DEFAULTS)
      {
      ...
      else if (*p_write->source == RS_RESTART_HIDDEN)
      {
          result = ffres_restart_hidden();
      }

      else if (*p_write->source == RS_RESTART_WITH_DEFAULTS)
      {
        ...
        rb_HasRun = false;
        #endif
        result = ffres_restart_restore_advanced();
      }
      //used for watch dog test.
      else if (*p_write->source == RS_RESTART_TEST_WATCHDOG)
      {
          wdog_WaitExternalWatchdog();
      }
      else if (*p_write->source == RS_RESTART_TEST_IWDOG)
      {
            mn_disable();
            Reset_Watchdog();         // feed external and internal
  ...
  return (result);
} //lint !e818 can't change the protoype since the function is plugged in a struct (fbs_call_fdev_funct)
...
FUNCTION_BODY
  ...
  appl_dia_update_field_diagnostics ();
    ...
    if(error_IsFault(FAULT_FW_CHECKSUM))
    {
        ...
        // restore only when device reset
        MN_ENTER_CRITICAL();
            p_RESB->block_err |= BLK_ERR_MEMORY_FAILURE;
        MN_EXIT_CRITICAL();
    }
  ...
  /* Update flash self-test monitor status of the resource block error, double defence again -- */
    ...
    if(error_IsFault(FAULT_TMOUT_FLASH_TEST))
    {
        MN_ENTER_CRITICAL();
            p_RESB->block_err |= BLK_ERR_DEV_NEEDS_MAINT_NOW;
        MN_EXIT_CRITICAL();
    }
